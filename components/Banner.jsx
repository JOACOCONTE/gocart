...
export default function Banner() {
    ...
    const [isOpen, setIsOpen] = React.useState(banner.isVisible)

    React.useEffect(() => {
        setIsOpen(banner.isVisible)
    }, [banner.isVisible])
    ...
    return isOpen && (
        <div className={`w-full px-6 py-1 font-medium text-sm text-white text-center bg-gradient-to-r ${banner.gradient}`}>
            <div className='flex items-center justify-between max-w-7xl mx-auto'>
                <p>{banner.title}</p>
                <div className="flex items-center space-x-6">
                    <button onClick={handleClaim} type="button" className="font-normal text-gray-800 bg-white px-7 py-2 rounded-full max-sm:hidden">
                        {banner.buttonText}
                    </button>
